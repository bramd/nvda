...
Mirror `nvda_ia2`: a staticlib+rlib crate depending on `nvda_vbuf`, holding the
Acrobat interface bindings + the Acrobat fillVBuf + the backend state/externs.
Add it to scons's cargo `--package` list (`nvdaHelper/remote/sconscript`) so it
...
`nvda_ia2::gecko_backend_state::nvda_ia2_gecko_backend_update` as raw-pointer
code (drain → per-subtree `fill_vbuf` → `replace_subtrees`). It is
backend-agnostic except for the call to `fill_vbuf`. **Before touching Acrobat,
refactor this into a reusable form** so both backends share it and future
backends are "just port fillVBuf". Two shapes:

- **Preferred:** finish wiring `nvda_vbuf::backend`'s existing `Renderer` trait +
...
critically — read back from the **old** subtree's parent during a partial
...
nodes) carry the `"language"` attribute so the inheritance read works; if the
...
as Rust structs threaded through the render (as gecko threads its table state).
